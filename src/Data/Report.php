...
class Report {
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT shipping_state AS state,
				 ...
				 FROM {$wpdb->prefix}sbsch_order_state
	...
	private function add_labels_and_currency( array $rows, $country ) {
		...
		foreach ( $rows as $index => $row ) {
			...
			$rows[ $index ]['state_name'] = isset( $state_names[ $code ] ) && $state_names[ $code ]
				? $state_names[ $code ]
				: ( '' === $code ? __( 'Unknown', 'sales-by-state-report-for-charitable' ) : $code );
	...
	private function money( $amount ) {
		...
		if ( function_exists( 'charitable_format_money' ) ) {
			$formatted = charitable_format_money( (string) $amount );
