...
namespace SBSCH\Install;

use SBSCH\Data\OrderSource;
...
class Schema {
	...
	const DB_VERSION     = 1;
	const VERSION_OPTION = 'sbsch_db_version';
	...
	public static function table() {
		...
		return $wpdb->prefix . 'sbsch_order_state';
	}
	...
	public static function install() {
		...
		$sql = "CREATE TABLE {$wpdb->prefix}sbsch_order_state (
		...
		if ( 0 === $counts['rows'] ) {
			delete_option( 'sbsch_backfill_cursor' );
		}
	...
	public static function table_exists() {
		...
		return (bool) $wpdb->get_var(
			$wpdb->prepare( 'SHOW TABLES LIKE %s', $wpdb->prefix . 'sbsch_order_state' )
		);
	}
	...
	public static function counts() {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		$rows = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}sbsch_order_state" );
	...
	public static function truncate() {
		...
		$wpdb->query( "TRUNCATE TABLE {$wpdb->prefix}sbsch_order_state" );
	}
}
