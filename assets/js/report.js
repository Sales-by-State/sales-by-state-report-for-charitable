...
( function ( wp, config ) {
	...
	var API = '/sbsch/v1/report';
	var DIAGNOSTICS = '/sbsch/v1/diagnostics';
	var BACKFILL = '/sbsch/v1/backfill';
	var DEFAULT_PER_PAGE = 25;
	var TEXTDOMAIN = 'sales-by-state-report-for-charitable';
	...
	function ImportStatus( props ) {
		...
		if ( ! diag.table_exists ) {
			return el(
				'div',
				{ className: 'sbsch-notice is-error' },
				el( 'p', { className: 'sbsch-notice__title' }, __( 'The report table is missing.', TEXTDOMAIN ) ),
		...
		return el(
			'div',
			{ className: 'sbsch-importing', role: 'status', 'aria-live': 'polite' },
			el(
				'div',
				{ className: 'sbsch-importing__row' },
				el( 'span', { className: 'sbsch-importing__label' }, __( 'Reading past donations…', TEXTDOMAIN ) ),
				el( 'span', { className: 'sbsch-importing__count' }, pct + '%' )
			),
			el(
				'div',
				{ className: 'sbsch-progress' },
				el( 'div', { className: 'sbsch-progress__bar', style: { width: pct + '%' } } )
			),
			el(
				'p',
				{ className: 'sbsch-importing__hint' },
	...
	function readStatuses( query ) {
		var raw = query && query.sbschStatuses;
	...
	/**
	 * Compare Charitable status IDs without number/string mismatches.
	 *
	 * @param {*} a First ID.
	 * @param {*} b Second ID.
	...
	function StatusFilter( props ) {
		...
		return el(
			'div',
			{ className: 'sbsch-filter' },
			el( 'span', { className: 'sbsch-filter__label' }, __( 'Donation status', TEXTDOMAIN ) ),
			el( Dropdown, {
				className: 'sbsch-filter__dropdown',
				contentClassName: 'sbsch-status-popover',
				...
				renderToggle: function ( toggleProps ) {
					return el(
						Button,
						{
							className: 'sbsch-filter__toggle',
				...
				renderContent: function () {
					return el(
						'div',
						{ className: 'sbsch-status-popover__list' },
	...
	function Filter( props ) {
		return el(
			'label',
			{ className: 'sbsch-filter' },
			el( 'span', { className: 'sbsch-filter__label' }, props.label ),
			el(
				'select',
				{
					className: 'sbsch-filter__select',
					value: props.value,
					onChange: function ( event ) {
	...
	function SalesByStateReport( props ) {
		...
		var country = query.sbschCountry || settings.defaultCountry || 'US';
		var year = query.sbschYear || settings.defaultYear;
		...
		var children = [
			...
			el(
				'div',
				{ key: 'filters', className: 'sbsch-filters' },
				el( Filter, {
					label: __( 'Country', TEXTDOMAIN ),
					value: country,
					options: settings.countries || [],
					onChange: function ( value ) {
						setFilter( { sbschCountry: value } );
					}
				} ),
				el( Filter, {
					...
					onChange: function ( value ) {
						setFilter( { sbschYear: value } );
					}
				} ),
				el( StatusFilter, {
					value: statuses,
					onChange: function ( next ) {
						setFilter( { sbschStatuses: next.join( ',' ) } );
					}
				} )
			)
		];
		...
		if ( data.error ) {
			children.push(
				el(
					'div',
					{ key: 'error', className: 'notice notice-error sbsch-error' },
		...
		return el( 'div', { className: 'woocommerce-analytics__report sbsch-report' }, children );
	}
	...
	function readUrlQuery() {
		var query = {};
		var keys = [ 'sbschCountry', 'sbschYear', 'sbschStatuses', 'orderby', 'order', 'paged', 'per_page' ];
		...
		query.page = found.page || 'sbsch-sales-by-state';
	...
	function mountStandalone() {
		function render() {
			var node = document.getElementById( 'sbsch-root' );

			if ( ! node ) {
				return;
			}

			if ( node.getAttribute( 'data-sbsch-mounted' ) ) {
				return;
			}

			node.setAttribute( 'data-sbsch-mounted', '1' );
